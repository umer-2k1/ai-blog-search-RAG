...
from langchain_groq import ChatGroq  # Changed from Google Generative AI
...
if 'groq_api_key' not in st.session_state:  # Changed from gemini_api_key
...
def set_sidebar():
    ...
    with st.sidebar:
        ...
        groq_api_key = st.text_input("Enter your Groq API key:", type="password")  # Changed
  
        if st.button("Done"):
            if qdrant_host and qdrant_api_key and groq_api_key:  # Updated validation
...
def initialize_components():
    ...
    if not all([st.session_state.qdrant_host, 
               st.session_state.qdrant_api_key, 
               st.session_state.groq_api_key]):  # Updated validation
        return None, None, None

    try:
        # Initialize embedding model with Google API key (keeping Google for embeddings)
...
def main():
    ...
    st.markdown("---")
    st.write("Built with :blue-background[LangChain] | :blue-background[LangGraph] by [Muhammad Umar](https://github.com/umer-2k1)")  # Updated attribution
